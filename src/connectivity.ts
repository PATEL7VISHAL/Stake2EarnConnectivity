...
export class Connectivity {
  ...
  async __getMainStateInfo() {
    ...
    if (this.cacheNftInfos.size == 0) {
      let _nfts = [...Array.from(programDummyNfts), ...Array.from(programHRServerNfts), ...Array.from(userHRServerNfts)]
      let allNftsStr = new Set<string>();
      _nfts.map((e) => allNftsStr.add(e))

      // let allNftKey = Array.from(allNftsStr).map((e)=>new web3.PublicKey(e))
      let _NftsMetadataAccountKeys: web3.PublicKey[] = Array.from(allNftsStr).map((e) => this.__getMetadataAccount(new web3.PublicKey(e)))
      let _NftMetadataInfo = await this.connection.getMultipleAccountsInfo(_NftsMetadataAccountKeys)
